...
As I had never extracted data from my phone before, I found the instructions from the last link above was pretty helpful. The app data contained several sqlite databases, but the two relevant ones were record_database and trace_together_status. The author of the post used a kibana dashboard and an Elasticsearch database to visualize their data, but while I was familiar with manipulating Grafana dashboards from work, I had never set a server up by myself before. I thus decided to get onto more familiar ground by exporting the relevant extracted sqlite database as a csv file to examine it in a standard jupyter notebook.
